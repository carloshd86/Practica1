...
public:
	...
	Sprite(const ltex_t* tex, int hframes = 1, int vframes = 1) :
		...
		mCollider(nullptr),
		mFlipImageH(false),
		mFlipImageV(false){ setTexture(tex); updateCollisionInfo(); }
	...
	bool getFlipImageH() const { return  mFlipImageH; }
	bool getFlipImageV() const { return  mFlipImageV; }
	void setFlipImage(bool flipImageH, bool flipImageV) { mFlipImageH = flipImageH; mFlipImageV = flipImageV; }
	Vec2					mRectOrigin;
	Vec2					mRectSize;
protected:
